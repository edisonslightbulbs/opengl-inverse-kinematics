...
void Arm::calculatePosition()
{
    ...
    // computing new position similar to Jacobin construction
    for (auto & limb : limbs) {
        angle += limb->angle;
        currentPosition(0) += limb->length * std::cos(angle);
        currentPosition(1) += -limb->length * std::sin(angle);
    }
}

MatrixXf Arm::jacobian()
{
    // constructing the Jacobin for the linked structure
    ...
    for (int col = 0; col < limbs.size(); col++) {
        ...
        for (int i = 0; i < limbs.size(); i++) {
            angle += limbs[i]->angle;
            if (i >= col) {
                j(0, col) += (-limbs[i]->length * std::sin(angle));
                j(1, col) += (limbs[i]->length * std::cos(angle));
            }
        }
    }
    return j;
 ...
 * the top matrix is what all rendering commands use to transform their data. By
 * pushing and popping matrices, one can control what transformations apply to
 * which objects, as well as apply transformations to groups of objects, and
 * easily reverse the transformations so that they don't affect other objects.
 */
...
void Arm::draw()
{
    ...
    glTranslatef(
        armBase(0), armBase(1), 0.0f); // Applies translation to matrix 1
    glutSolidSphere(3.0f, 8, 8);       // base | root joint

    // represent the joints by a sphere (on the tips of every arm-structure)
    for (auto & limb : limbs) {
        glRotatef(degrees(limb->angle), 1.0f, 0.0f, 0.0f);
        limb->color.apply();
        gluCylinder(limb->mObj, 1, 1, limb->length, 20, 20);
        glTranslatef(0, 0, limb->length);
        glColor gColor = {0.5f, 0.5f, 0.5f };
        gColor.apply();
...
void Arm::update()
{
    ...
    float x = (targetPosition(0) - currentPosition(0) > 1) ? effectorStep
                                                           : -effectorStep;
    float y = (targetPosition(1) + currentPosition(1) > 1) ? effectorStep
                                                           : -effectorStep;
...
{
    if (std::fabs(currentPosition(0) - targetPosition(0)) <= 2.0f
        && std::fabs(currentPosition(1) + targetPosition(1)) <= 2.0f) {
        resolveTarget = false;
        return true;
    }
    return false;
