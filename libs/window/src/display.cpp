...
// todo: target point should be determined by the position of the mouse
...
void glut_update()
{
    glutPostRedisplay(); // mark current window for re-displaying
}
...
void glut_timer(int32_t i) // 10ms call
{
    ...
    if (arm.isTargetResolved()) {
        targetPoint = arm.getPointWithinRange();
        arm.moveToPoint(targetPoint);
    }
